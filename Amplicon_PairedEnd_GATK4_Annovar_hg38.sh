...
Exome_bed='Amplicon_Run7.bed'
Exome_Vcfbed='Amplicon_Run7.bed'
...
N=22 ; Number of jobs can be run parallel 
threads=5
...
mosdepth -x --by ${Exome_bed} QC/AlignmentQC/Mosdepth/Exome/${Samplename}_Exome BamFiles/${Samplename}_recal.bam \
--thresholds 1,10,20,30,50,100  -F 772 &


mkdir -p QC/AlignmentQC/flagstat
samtools flagstat BamFiles/${Samplename}_recal.bam > QC/AlignmentQC/flagstat/${Samplename}_flagstat.txt &

done < Sample_Names.txt ; wait



${Mosedepth_plots}  QC/AlignmentQC/Mosdepth/Exome/*.dist.txt --output QC/AlignmentQC/Mosdepth/Exome/All_MosdepthCoverage.html 



mkdir -p MultiQC/AlignmentQC/Mosdepth/Exome
multiqc -o MultiQC/AlignmentQC/Mosdepth/Exome/ QC/AlignmentQC/Mosdepth/Exome
...
#while read Samplename ; do ((i=i%N)); ((i++==0)) && wait
#	        vcffilter -f "FILTER = PASS" "${Samplename}_"hardfiltered_Target.vcf  > "${Samplename}_"Target_Pass_Illumina.vcf ; done < Sample_Names.txt ; wait
...
rm -rf fasqfile.txt temp.txt *_hardfiltered.vcf.gz* *idx
